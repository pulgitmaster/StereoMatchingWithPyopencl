...
class ZNCC():
    def __init__(self, kernel_source_path=None, VERBOSE=True):
        ...
        src_path = os.path.join(os.getcwd(), 'ZNCC.cl')
        ...
        # kerner function
        self.get_mean = prg.get_mean
        self.get_std = prg.get_std
        self.get_cost = prg.get_cost
        self.wta = prg.wta

    def get_costvolume(self, left_img, right_img, kernel_size=5, dispRange=64, zncc_thread_num=(16, 16, 4), host_mem=False):
        ...
        mean_thread_num = None
        if zncc_thread_num is not None:
            assert w % zncc_thread_num[0] == 0 and h % zncc_thread_num[1] == 0 and dispRange % zncc_thread_num[2] == 0
            mean_thread_num = (zncc_thread_num[0], zncc_thread_num[1])
        std_thread_num = mean_thread_num
        ...
        left_mean_image2D = cl.Image(ctx, mf.READ_WRITE, fmt, shape=(w, h))
        right_mean_image2D = cl.Image(ctx, mf.READ_WRITE, fmt, shape=(w, h))
        left_std_image2D = cl.Image(ctx, mf.READ_WRITE, fmt, shape=(w, h))
        right_std_image2D = cl.Image(ctx, mf.READ_WRITE, fmt, shape=(w, h))
        ...
        self.get_mean(
            queue,
            (w, h), # global size
            (mean_thread_num), # local size
            left_image2D, right_image2D, # input
            left_mean_image2D, right_mean_image2D, # output
            var_ksize
        ).wait()
        ...
        self.get_std(
            queue,
            (w, h), # global size
            (std_thread_num), # local size
            left_image2D, right_image2D, # input
            left_mean_image2D, right_mean_image2D, # input
            left_std_image2D, right_std_image2D, # output
            var_ksize
        ).wait()
        ...
        self.get_cost(
            ...
            (zncc_thread_num), # local size
            left_image2D, right_image2D, # input
            left_mean_image2D, right_mean_image2D, # input
            left_std_image2D, right_std_image2D, # input
            cost_buffer, # output
            var_width,
            var_ksize,
            var_dispRange
    ...
    def get_depth(self, left_img, right_img, kernel_size=5, dispRange=64, zncc_thread_num=(16, 16, 4), wta_thread_num=(16, 16), host_mem=False):
        # arguments
        h = left_img.shape[0]
        w = left_img.shape[1]
        var_height = np.int32(h)
        ...
        cost_results = self.get_costvolume(
            ...
            dispRange=dispRange,
            zncc_thread_num=zncc_thread_num,
            host_mem=host_mem
        )
        if host_mem:
...
if __name__ == '__main__' :
    ...
    with_cost = False
    st_time = time.time()
    result = zncc.get_depth(
        left_img,
        right_img,
        kernel_size=5,
        dispRange=dispRange,
        zncc_thread_num=None,
        wta_thread_num=None,
        host_mem=with_cost)
